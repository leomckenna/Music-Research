...
QUERY_LIMIT = 5   # ✅ Limit number of queries during testing
MAX_RESULTS = 5 if TEST_MODE else 50  # ✅ Limit API results during testing
...
def collect_control_videos():
    ...
    queries_to_run = SEARCH_QUERIES[:QUERY_LIMIT] if TEST_MODE else SEARCH_QUERIES

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        executor.map(lambda q: process_query(q, processed_video_ids), queries_to_run)
...
if __name__ == "__main__":
    ...
    if not TEST_MODE:
        ...
        print("🚧 Test mode enabled — Skipping WAV downloads.")
